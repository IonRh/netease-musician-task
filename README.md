...
- ✅ **公共互助听歌**：使用服务端独立 Token 和一次性任务凭证，支持歌曲/专辑共享播放
- ✅ **本地账号互助**：本机已登录账号之间轮换播放，不上传账号、歌曲和统计到第三方互助池
- ✅ **听歌配额控制**：支持每日和每月听歌上限，账号概览显示今日/当月听歌及被听进度
...
1. 从互助服务端获取独立客户端 Token，在「全局设置」中填写 API 地址、Token、歌曲/专辑 ID 和听歌上限。
2. 在账号列表中点击「听歌配置」→「加入听歌」。
3. 加入后，账号会在设定时间自动执行听歌任务。
4. 在「听歌配置」中可以查看听歌和被听进度，也可以退出共享听歌。

本地互助：在账号「编辑」中开启「加入本机账号互助」，并为音乐人账号填写希望被播放的歌曲 ID。执行账号会在其他本地参与账号之间轮换目标，播放和统计只保存在本机 SQLite。

公共互助和自动播放都可能触发网易云账号风控。请确认互助服务可信、合理设置频率，并自行承担使用风险。
