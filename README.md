...
- ✅ **互助听歌**：支持歌曲/专辑加入听歌，自动从 `全局配置中的API地址获取听歌信息`
...
1. 在「全局设置」中填写听歌 API 地址、歌曲/专辑 ID、听歌开始时间以及每日/月度听歌上限。
2. 在账号列表中点击「听歌配置」→「加入听歌」。
3. 加入后，账号会在设定时间自动执行听歌任务。
4. 在「听歌配置」中可以查看听歌和被听进度，也可以退出共享听歌。
5. API开源地址：[netease-musician-task-server-api](https://github.com/IonRh/netease-musician-task-server-api)
